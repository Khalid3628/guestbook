...
if (isset($_POST['upload'])) {
    ...
    include "config.php";
    $name = htmlentities(trim($_POST['tnama']));
    ...
    $ip = $ip1 . '-' . $ip2;
    if ((empty($name)) || (empty($email)) || (empty($komentar))) {
        echo "Data tidak boleh kosong";
        exit;
    } else {
        $query = "insert into pengunjung (name,email,komentar,date,ip)" . 
        "VALUES ('$name','$email','$komentar','$date','$ip')";
